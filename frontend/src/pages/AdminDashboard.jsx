    ...
    if (user?.role !== 'admin') {
        return (
            <div className="min-h-screen bg-brand-dark flex items-center justify-center p-4">
                <div className="bg-white border border-red-100 p-8 rounded-3xl max-w-md text-center shadow-xl">
                    <ShieldAlert className="w-16 h-16 text-red-500 mx-auto mb-4" />
                    <h2 className="text-2xl font-black text-gray-900 mb-2">Access Denied</h2>
                    <p className="text-gray-500">Strictly for system administrators only. Your attempt has been logged. 🛡️</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="flex bg-brand-dark min-h-screen text-gray-900 font-sans selection:bg-brand-yellow selection:text-white">
            ...
            <div className="flex-1 md:ml-64 p-8 transition-all duration-300">
                <header className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                    <div>
                        <div className="flex items-center gap-2 mb-1">
                            <span className="bg-brand-yellow text-white text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-tighter">Admin Panel</span>
                            <h1 className="text-3xl font-black text-gray-900">System Controller</h1>
                        </div>
                        <p className="text-gray-500 text-sm italic">Monitoring FinCore infrastructure in real-time. ⚡</p>
                    </div>

                    <div className="flex items-center gap-4">
                        <button
                            onClick={() => setShowAdminModal(true)}
                            className="bg-brand-yellow text-white px-6 py-2.5 rounded-xl font-black text-xs uppercase tracking-widest hover:bg-blue-700 transition flex items-center gap-2"
                        >
                        ...
                        <div className="flex items-center bg-white border border-gray-100 rounded-2xl px-4 py-2 shadow-sm">
                            <div className="flex flex-col items-end mr-4">
                                <span className="text-xs text-gray-400 font-bold uppercase">Superuser</span>
                                <span className="text-sm font-black text-gray-900">{user?.fullName}</span>
                            </div>
                            <div className="w-10 h-10 rounded-full bg-brand-yellow flex items-center justify-center text-white font-black">
                ...
                {/* Tab Navigation */}
                <div className="flex space-x-2 mb-8 bg-gray-50 p-1 rounded-2xl border border-gray-100 w-fit">
                ...
                {loading ? (
                    ...
                    <div className="animate-fade-in-up">
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                    ...
                                    <div className="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm flex flex-col justify-center items-center text-center">
                                        ...
                                        <h3 className="text-xl font-black text-gray-900 mb-2">Asset Distribution</h3>
                                        <p className="text-gray-500 text-sm max-w-xs">84% of total volume is concentrated in Savings accounts. Review liquidity protocols.</p>
                                        <button className="mt-8 border border-brand-yellow text-brand-yellow px-6 py-2 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-brand-yellow hover:text-white transition">Download Full Report</button>
                                    </div>
                                </div>
                            </div>
                        )}
                        ...
                        {activeTab === 'users' && (
                            <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm">
                                <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
                                    <h3 className="text-xl font-black text-gray-900">Registered Users ({users.length})</h3>
                                    <div className="relative w-full md:w-64">
                                        <input
                                            type="text"
                                            placeholder="Search users..."
                                            className="w-full bg-gray-50 border border-gray-100 rounded-xl py-2 pl-10 pr-4 text-sm focus:outline-none focus:border-brand-yellow"
                        ...
                        {activeTab === 'accounts' && (
                            <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm">
                                <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
                                    <h3 className="text-xl font-black text-gray-900">Financial Assets ({accounts.length})</h3>
                                    <div className="relative w-full md:w-64">
                                        <input
                                            type="text"
                                        ...
                                        <tbody className="divide-y divide-gray-50">
                                            {accounts.filter(a => a.accountNumber.includes(searchTerm)).map((a) => (
                                                <tr key={a._id} className="hover:bg-gray-50 transition-colors">
                                                    <td className="px-6 py-4 font-mono text-xs text-brand-yellow">{a.accountNumber}</td>
                                                    <td className="px-6 py-4">
                                                        <div className="font-bold text-sm text-gray-900">{a.userId?.fullName || 'N/A'}</div>
                                                        <div className="text-[10px] text-gray-500">{a.accountType}</div>
                                                    </td>
                                                    <td className="px-6 py-4 font-black text-sm text-gray-900">₹{a.balance.toLocaleString()}</td>
                                                    <td className="px-6 py-4">
                        ...
                        {activeTab === 'transactions' && (
                            <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm">
                                <div className="p-6 border-b border-gray-100 flex flex-col md:flex-row justify-between items-center gap-4">
                                    <h3 className="text-xl font-black text-gray-900">Master Ledger ({transactions.length})</h3>
                                    <button className="bg-blue-50 text-brand-yellow px-4 py-2 rounded-xl text-xs font-black uppercase flex items-center gap-2">
                                        ...
                                        <tbody className="divide-y divide-gray-50">
                                            {transactions.map((tx) => (
                                                <tr key={tx._id} className="hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="font-bold text-sm uppercase text-gray-900">{tx.type}</div>
                                                        <div className="text-[10px] text-gray-500 truncate max-w-[150px]">{tx.description}</div>
                                                    </td>
                                                    <td className="px-6 py-4 font-black text-gray-900">₹{tx.amount.toLocaleString()}</td>
                                                    <td className="px-6 py-4">
                                                        <span className="text-[10px] font-bold text-gray-400 uppercase">{tx.transactionMode}</span>
                                                    </td>
                                                    <td className="px-6 py-4">
                                                        <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${tx.status === 'success' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'
                                                            }`}>{tx.status}</span>
                                                    </td>
                                                    <td className="px-6 py-4 text-[10px] text-gray-400">{new Date(tx.createdAt).toLocaleString()}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                    </table>
            ...
            {showAdminModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm">
                    <div className="bg-white border border-gray-100 p-8 rounded-[2rem] w-full max-w-md animate-fade-in-up shadow-2xl">
                        <div className="flex justify-between items-center mb-6">
                            <h3 className="text-2xl font-black text-gray-900">Create Administrator</h3>
                            <button onClick={() => setShowAdminModal(false)} className="text-gray-400 hover:text-gray-900 transition"><Lock size={20} /></button>
                        </div>
                        <form onSubmit={handleCreateAdmin} className="space-y-4">
                            <div>
                                ...
                                <input
                                    type="text"
                                    required
                                    className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-gray-900 focus:outline-none focus:border-brand-yellow placeholder-gray-400"
                            ...
                            <button type="submit" className="w-full bg-brand-yellow text-white font-black py-4 rounded-2xl hover:bg-blue-700 transition transform active:scale-95 uppercase tracking-widest mt-4 shadow-lg shadow-brand-yellow/30">
                                Initialize Admin
                            </button>
                        </form>
                    </div>
...
const StatCard = ({ icon, title, value, color }) => {
    ...
    return (
        <div className="bg-white p-6 rounded-3xl border border-gray-100 hover:bg-gray-50 transition group shadow-sm">
            ...
            <h3 className="text-2xl font-black text-gray-900 mt-1">{value || '0'}</h3>
        </div>
    );
};
...
const LogItem = ({ type, msg, time }) => {
    ...
    return (
        <div className="flex items-start gap-4 p-4 hover:bg-gray-50 rounded-2xl transition">
            ...
            <div className="flex-1">
                ...
                <span className="text-[10px] text-gray-400 flex items-center gap-1 mt-1 font-bold italic"><Clock size={10} /> {time}</span>
            </div>
        </div>
    );
};
