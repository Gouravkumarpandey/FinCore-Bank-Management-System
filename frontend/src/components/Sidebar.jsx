...
const Sidebar = () => {
    ...
    return (
        <div
            className={`hidden md:flex flex-col h-screen bg-white border-r border-gray-100 shadow-xl fixed left-0 top-0 z-50 transition-all duration-300 ${isCollapsed ? 'w-20' : 'w-64'}`}
        >
            {/* Header / Collapse Toggle */}
            <div className={`flex items-center h-24 border-b border-gray-100 relative ${isCollapsed ? 'justify-center' : 'justify-start pl-6'}`}>
                <Link to="/" className="flex items-center group">
                    <Zap className={`text-brand-yellow transition-all ${isCollapsed ? 'h-8 w-8' : 'h-8 w-8 mr-3group-hover:scale-110'}`} />
                    {!isCollapsed && <span className="ml-3 text-2xl font-black text-gray-900 tracking-tight animate-fade-in">FinCore.</span>}
                </Link>

                <button
                    onClick={toggleSidebar}
                    className="absolute -right-3 top-10 bg-brand-yellow text-white p-1 rounded-full shadow-lg hover:scale-110 transition z-50 border-2 border-white"
                >
            ...
            <div className="flex-1 flex flex-col overflow-y-auto px-2 py-6 space-y-2 scrollbar-none" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
                ...
                {menuItems.map((item) => (
                    <Link
                        ...
                        className={`group flex items-center py-3 text-sm font-bold rounded-xl transition-all duration-200 relative ${isActive(item.path)
                            ...
                            : 'text-gray-500 hover:bg-gray-50 hover:text-brand-yellow'
                            } ${isCollapsed ? 'justify-center px-0 mx-2' : 'px-4 mx-2'}`}
                        title={isCollapsed ? item.name : ''}
                    >
                        <item.icon className={`transition-colors ${isCollapsed ? 'h-6 w-6' : 'h-5 w-5 mr-3'} ${isActive(item.path) ? 'text-white' : 'group-hover:text-brand-yellow text-gray-400'}`} />
                ...
                <div className={`pt-6 pb-2 ${isCollapsed ? 'border-t border-gray-100 mx-2 mt-4' : ''}`}>
                    ...
                    {secondaryItems.map((item) => (
                        <Link
                            ...
                            title={isCollapsed ? item.name : ''}
                        >
                            <item.icon className={`transition-colors ${isCollapsed ? 'h-6 w-6' : 'h-5 w-5 mr-3'} text-gray-400 group-hover:text-brand-yellow`} />
                            {!isCollapsed && <span className="truncate">{item.name}</span>}
                        </Link>
                    ))}
                </div>
